...
if __name__ == "__main__":
    ...
    for count, country in enumerate(countries_unique):
        ...
        year_unique = np.unique(metadata_df.loc[metadata_df.index[idx_country], "year"])
    ...
    if not os.path.exists(results_folder + "/Correlations"):
        os.makedirs(results_folder + "/Correlations")

    writer = pd.ExcelWriter(results_folder + "/Correlations/"+specie_name+".xlsx", engine='xlsxwriter')
    ...
    for count, class_name in enumerate(class_RSI_df.columns):
        ...
        for name_antibiotic in amr_df.columns:
            ...
            # Get features
            features_file = results_folder + "/"+specie_name+"_AMR/Final_FullData_Model/"+specie_name+"_"+name_antibiotic+"_FULL_coefficients.csv"                
            
            if not os.path.isfile(features_file):
                print(f"File not found: {features_file}. Skipping...")
